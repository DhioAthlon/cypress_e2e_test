describe('Cypress Playground Tests', () => {
  ...
  it('Clicks the Subscribe button and verifies the action', () => {
   ...
   it('Types a name in the Sign here field and verifies the signature preview', () => {
    cy.get('#signature-textarea')
      .type('Dhio Athlon'); 
    cy.get('#signature')
      .should('have.text', 'Dhio Athlon'); //bisa pakek contain dan have.text, have.text lebih ketat sampai spasi
  });
  
  it('Should type name and check the signature preview checkbox', () => {
    cy.get('#signature-textarea-with-checkbox')
      .type('Dhio Athlon'); 
    cy.get('input#signature-checkbox')
      .check();  // contohnya pakek [type="checkbox"] tapi ga spesifik checkbox yg signature checkbox, soalnya ada dua checkbox di testnhya 
   ...
   it('should check the On/Off radio button and display the correct text', () => {
    cy.get('#on')
      .check(); //contohnya pakek input[type="radio"] tapi malah error anjir, harus ada id/value, nulisnya  cy.get('input[type="radio"][value="on"]').check(); tapi kalok ada value/id langsung aja gausah input type radio lg soalnya udah spesifik
    cy.get('#on')
      .should('be.checked');
    cy.contains('ON')
      .should('be.visible');
  ...
  it('Intercepts the GET request and displays the TODO item', () => {
  // Intercept the GET request
  cy.intercept('GET', '**/todos/1')
    .as('getTodo'); //gapaham kok 'https://api.example.com/todos/1'). error, terus cari yg bener gimana juga bingung
  ...
  // Assert that the TODO item is displayed
  cy.contains('TODO ID: 1').should('be.visible');
  cy.contains('Title: delectus aut autem').should('be.visible');
  cy.contains('Completed: false').should('be.visible');
  cy.contains('User ID: 1').should('be.visible');
});

it('Uses a fixture as the response for the GET request', () => {
  // Intercept the request and respond with the fixture
  cy.intercept('GET', '**/todos/1', { fixture: 'todo' }).as('getTodo')
  cy.contains('button', 'Get TODO').click() // this would trigger the above request, but instead of making a real API call, a static file defined inside of the cypress/fixtures/ directory would be returned.
  cy.wait('@getTodo')

  // Assert the TODO item from the fixture is displayed
  cy.contains('TODO ID: 1').should('be.visible');
  cy.contains('Title: Fixture TODO Title').should('be.visible');
  cy.contains('Completed: true').should('be.visible');
  cy.contains('User ID: 1').should('be.visible');
});

it('Simulates a server failure and displays an error message', () => {
  // Intercept the request and respond with a 500 status code
  cy.intercept('GET', '**/todos/1', { statusCode: 500 }).as('serverFailure');
  
  // Click the "Get TODO" button
...
it('Simulates a network failure and displays a fallback message', () => {
  // Intercept the request and force a network error
  cy.intercept('GET', '**/1', { forceNetworkError: true }).as('networkError');
  ...
  // Assert that a fallback message is displayed
  ...
  it('Downloads a text file and verifies its content', () => {
    ...
    cy.readFile('cypress/downloads/example.txt')
      .should('be.equal', 'Hello, World!'); // Verify file content
  });
